...
var addToCartButtons = document.querySelector(".add-btn cart");
...
function bought() {
  var name = checkOutContainer.querySelector(
    ".checkout .table-layout .name"
  ).value;
  var address = checkOutContainer.querySelector(
    ".checkout .table-layout .address"
  ).value;
  if (name === "" || address === "") {
...
function copyCartSummary() {
  ...
  const checkoutContainer = document.querySelector(
    ".checkout-container .order-list"
  );
  const cartItemsList = cartSummary
    .querySelector("#cart-items")
    .cloneNode(true);

  const totalCheckout = document.getElementById("totalCheckout");
  totalCheckout.innerHTML =
    "Php " + document.getElementById("cart-total").textContent;

  const total = document.getElementById("totalOverall");
  const delivery = document.getElementById("delivery");

  const totalCheckoutValue = parseFloat(
    totalCheckout.textContent.replace("Php", "").trim()
  );
  const deliveryValue = parseFloat(
    delivery.textContent.replace("Php", "").trim()
  );
